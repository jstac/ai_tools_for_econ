...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \vspace{0.5em}
        \item What if we can't convert problems to array-processing operations?
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Everything compiled might not be optimal (e.g., debugging)
        \vspace{0.2em}
...
\begin{frame}
    ...
    ``The acronym JAX stands for \brown{Just After eXecution}, since to com-
    ...
    \pause
    Parallelization is a major focus\ldots
...
\begin{frame}{Summary from Google Gemini}
    ...
    JAX's JIT compiler acts like a master engineer 
        ...
        \vspace{0.5em}
    \begin{itemize}
        \item Designs a custom assembly line using these tools
        \vspace{0.5em}
        \item Precisely tailored to the dimensions of the specific ``parts'' (array shapes) being processed
    \end{itemize}
...
\begin{frame}
    ...
    \begin{figure}[h!]
    \centering
    \scalebox{0.5}{\input{compiler_diagram.tex}}
\end{figure}
...
def add_tax(prices):
    ...
    \vspace{0.5em}
    \begin{itemize}
        \item Accesses global state (\texttt{tax\_rate})
        \item Has side effects (modifies \texttt{prices} -- outside function's scope)
    \end{itemize}
