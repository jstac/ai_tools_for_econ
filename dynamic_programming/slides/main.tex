...
\begin{frame}{Prelude: some linear algebra}
    ...
    \begin{itemize}
        \item $K$ and $b$ are given, while $v$ is the unknown
            \medskip
        \item $v$ maps finite set $\Xsf$ to $\RR$  (we write $v \in \RR^\Xsf$)
            \medskip
        \item Our objective is to solve the equation for $v$
    \end{itemize}
...
\begin{frame}[fragile]
    ...
    %
    \begin{equation*}
        \rho(K) \coloneq \max \setntn{|\lambda|}{\lambda \text{ is an eigenvalue of } K}
    \end{equation*}
...
\begin{frame}
   ...
   The next result is the matrix analogue of 
   %
   \begin{equation*}
       |a| < 1
       \quad \implies \quad
...
\begin{frame}{Prelude: Markov chains}

    Let
    %
    \begin{itemize}
        \item $\Xsf$ be a finite set, as before
            \medskip
        \item $(X_t)$ be a stochastic process taking values in $\Xsf$
            \medskip
        \item $P$ be a stochastic matrix ($P \geq 0$ and $P \1 = \1$)
    \end{itemize}
    ...
    %
    \begin{equation*}
        \text{$\PP\{X_{t+1} = x' \given X_t = x\} = P(x, x')$}
    \end{equation*}
    %
    for all $x, x' \in \Xsf$
...
\begin{frame}[fragile]
    ...
    \begin{minted}{python}
def generate_markov_chain(key, P, x_0, num_steps):
    " Generate a P-Markov chain with initial condition x_0. "
    n = P.shape[0]
    chain = jnp.zeros(num_steps + 1, dtype=jnp.int32)
    chain = chain.at[0].set(x_0)
    
    def update(t, carry):
        key, chain = carry
        key, subkey = random.split(key)
        x = chain[t]
        y = random.choice(subkey, n, p=P[x, :])
        chain = chain.at[t + 1].set(y)
        return (key, chain)
    
    final_key, final_chain = jax.lax.fori_loop(
        0, num_steps, update, (key, chain)
    )
    return final_chain
\end{minted}
...
\begin{frame}\frametitle{Introduction to DPs: the Affine Case}
    ...
    We consider dynamic programs operating on 
    %
    \begin{enumerate}
...
\begin{frame}
    ...
    \begin{itemize}
        \item the \navy{reward function} $r$ maps $\Gsf$ to $\RR$ and
            \medskip
        \item the \navy{transition kernel} $K$ maps $\Gsf \times \Xsf$ to $\RR_+$ 
    \end{itemize}
...
\begin{frame}
    ...
    What's the intuition behind the Bellman equation
    %
    \begin{equation*}
        v(x)
        = \max_{a \in \Gamma(x)}
    ...
    \medskip
    Think of $v(x')$ as maximum lifetime reward from future state $x'$

    The max trades off
    %
    \begin{enumerate}
        \item current reward $r(x, a)$
            \medskip
        \item expected discounted future reward $\sum_{x'} v(x') K(x, a, x')$
    \end{enumerate}

    \medskip
    \medskip

    The result yields maximum lifetime reward $v(x)$
...
\begin{frame}
    ...
    \begin{figure}
       \centering
       \scalebox{0.4}{\includegraphics{figures/plot_interest_rates_nominal.pdf}}
...
\begin{frame}
    ...
    \begin{figure}
       \centering
       \scalebox{0.4}{\includegraphics{figures/plot_interest_rates_real.pdf}}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \medskip
        \item feasible correspondence $\Gamma$ is nonempty
    \end{itemize}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \scalebox{0.5}{\includegraphics{bellman_envelope.pdf}}
...
\begin{frame}
    ...
    Assume for now that $\rho(K_\sigma) < 1$ for all $\sigma$
    ...
    \begin{equation*}
        v_\sigma 
        := \sum_{t \geq 0} K_\sigma^t r_\sigma
        = (I - K_\sigma)^{-1} r_\sigma
    \end{equation*}
...
\begin{frame}
    ...
    Hence $v_\sigma$ satisfies
    %
    \begin{equation*}
        v_\sigma(x) 
            = r(x, \sigma(x))
            + \sum_{x'} v_\sigma(x') K(x, \sigma(x), x')
    \end{equation*}

    \medskip

    This is a natural recursion for the lifetime value of $\sigma$ 
...
\begin{frame}
    ...
    \begin{itemize}
        \item $\sigma$ extracts maximum lifetime value from all states
    \end{itemize}
...
\begin{frame}
    ...
    Consider an affine DP with Bellman operator $T$ and value function $v^*$
    \medskip

    \Thm If $\rho(K_\sigma) < 1$ for all $\sigma \in \Sigma$, then
    %
    \begin{enumerate}
        ...
        \vspace{0.5em}
        \vspace{0.5em}
        \item $T^n v \to v^*$ as $n \to \infty$ for all $v \in \RR^\Xsf$
        \vspace{0.5em}
        \vspace{0.5em}
        \item A feasible policy is optimal if and only if it is $v^*$-greedy
        \vspace{0.5em}
        \vspace{0.5em}
...
\begin{frame}
    ...
    \begin{figure} 
        \centering
        \scalebox{0.48}{\includegraphics{howard_newton_1.pdf}}
...
\begin{frame}
    ...
    \Prop Under the stated conditions, 
    %
    \begin{enumerate}
        \item VFI, HPI and OPI all converge, and
            \medskip
        \item HPI converges to an exact optimal policy in finitely many steps
    \end{enumerate}

    \vspace{0.4em}
    \vspace{0.4em}
    For details and proofs see \url{https://dp.quantecon.org/}
